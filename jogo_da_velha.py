...
def jogar(*args, **kwargs):
    prompt = Prompt()
    pos = prompt.ask("[b yellow]Escolha a Posição:[/]", 
                     choices=["1","2","3","4","5",
                              "6","7","8","9"]).upper().strip()
    
    valor = prompt.ask("[b yellow]Símbolo: [/]"
                       , choices=["x", "o"]).upper().strip()
    return pos, valor
...
while True:
    ...
    if keyboard.is_pressed('Ctrl'):
        ...
        console = Console()
        Columns(
            console.print("### Até a próxima noob :snake: :thumbsup:",
                      style="red  blink",
                      justify="center",),
                      align="center")
        sys.exit() 
...
# talvez mensagens de motivaçãos ofensiva kk: vove é muito ruim, o jogador x é seu fregues?